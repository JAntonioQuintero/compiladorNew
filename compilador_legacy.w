...
IF PROVERSION BEGINS "9" THEN ASSIGN lvc_verDir = "".
...
CASE pc-sistema:
    ...
    OTHERWISE DO:
        ...
        ASSIGN c-win:TITLE = "Compilador -- (DESCONOCIDO)".
    END.
END CASE.
...
FOR EACH tt-temp:
  ...
  IF file-info:FILE-TYPE BEGINS "F" THEN DO:
    ...
    /*MCR: no toma en cuenta las utilerías*/
    IF tx-linea MATCHES "*\src\util\*" THEN
       NEXT.
...
FOR EACH tt-pfuentes
  ...
  CASE tipoprg:
    WHEN "W" OR WHEN "P" OR WHEN "I" OR WHEN "wrx" THEN DO:
      NEXT.
    END.
    OTHERWISE DO:
      DELETE tt-pfuentes.
...
FOR EACH tt-pfuentes
    WHERE (tt-pfuentes.tipoprg = "W" OR tt-pfuentes.tipoprg = "P") NO-LOCK:
    CREATE tt-progs.
...
ASSIGN vc-error = "".
IF tt-compila.dispv6p = TRUE THEN DO:

    ASSIGN
        lvc_Linea = "COMPILE VALUE(" + V-PROGRAMA + 
        ") SAVE INTO VALUE(" + tt-compila.rutades + 
        ") V6FRAME USE-REVVIDEO NO-ERROR.".
    IF tt-compila.rutades = "" OR tt-compila.rutades = ? THEN
        ASSIGN lvc_Linea = "NO COMPILA " + lvc_Linea.
    RUN Bitacora(INPUT lvc_Linea).
    IF tt-compila.rutades = "" OR tt-compila.rutades = ? THEN RETURN.

    COMPILE VALUE(V-PROGRAMA) SAVE INTO 
        VALUE(tt-compila.rutades) V6FRAME USE-REVVIDEO NO-ERROR.
END.
ELSE DO:

    ASSIGN 
        lvc_Linea = "COMPILE VALUE(" + V-PROGRAMA + 
        ") SAVE INTO VALUE(" + tt-compila.rutades + ") NO-ERROR.".
    IF tt-compila.rutades = "" OR tt-compila.rutades = ? THEN
        ASSIGN lvc_Linea = "NO COMPILA " + lvc_Linea.
    RUN Bitacora(INPUT lvc_Linea).
    IF tt-compila.rutades = "" OR tt-compila.rutades = ? THEN RETURN.

    COMPILE VALUE(V-PROGRAMA) SAVE INTO VALUE(tt-compila.rutades) NO-ERROR.
END.
...
IF COMPILER:ERROR THEN DO:    
    ...
    ASSIGN
        lvc_Linea = 
        "Error de compilacion en: " + v-programa + " Linea: " +
        STRING(COMPILER:ERROR-ROW) + " Columna: " + STRING(COMPILER:ERROR-COL).
    RUN Bitacora(INPUT lvc_Linea).
    
    OUTPUT TO VALUE(vc-logcomp) APPEND.
    MESSAGE
        SKIP(1)
        "Error de compilacion en" 
        v-programa /*COMPILER:FILENAME */
        "Linea" 
        COMPILER:ERROR-ROW 
        "columna" 
        COMPILER:ERROR-COL.
    ...
    DO iError = 1 TO ERROR-STATUS:NUM-MESSAGES:
        ASSIGN vc-error = SUBSTITUTE("&1 &2 &3",
                                     TRIM(vc-error),
                                     CHR(13),
                                     ERROR-STATUS:GET-MESSAGE(iError)).
        MESSAGE ERROR-STATUS:GET-MESSAGE(iError).

        ASSIGN
            lvc_Linea = ERROR-STATUS:GET-MESSAGE(iError).
        RUN Bitacora(INPUT lvc_Linea).

    END.
    OUTPUT CLOSE.

    ASSIGN lvc_Linea = SUBSTITUTE("&1 &2 &3 &4 &5 &6 &7",
                                 "Error de compilacion en",
                                 COMPILER:FILENAME,
                                 "linea",
                                 COMPILER:ERROR-ROW,
    ...
    ASSIGN lvc_Linea = lvc_Linea + " " + vc-error.
...
ELSE DO:
    ...
    /*Si es proyecto copia el compilado en la ruta original*/
    IF vi-proy <> 0 THEN DO:
...
------------------------------------------------------------------------------*/
FOR EACH tt-propath
  NO-LOCK BY tt-propath.rutapro DESC:
  IF LOOKUP(tt-propath.rutapro,PROPATH,",") = 0 THEN DO:
    ASSIGN PROPATH = SUBSTITUTE("&2,&1",PROPATH,tt-propath.rutapro).
  END.
END.

IF LOOKUP(vc-amb01,PROPATH,",") = 0 THEN DO:
  ASSIGN PROPATH = SUBSTITUTE("&2,&1",PROPATH,vc-amb01).
END.

IF LOOKUP(vc-amb02,PROPATH,",") = 0 THEN DO:
  ASSIGN PROPATH = SUBSTITUTE("&2,&1",PROPATH,vc-amb02).
END.
...
FOR EACH tt-progs
    ...
    FOR EACH tt-rutas
        ...
        IF PROVERSION BEGINS '11' THEN DO:
           RUN util\pTestV6Frame.p(tt-progs.rutaprg + tt-progs.nomprog + '.' + tt-progs.tipoprg,OUTPUT vl-v6frame).
        END.
